...
{
    /// <summary>
    /// Summary description for MailReceipt_PDFNew
    /// </summary>
    public class MailReceipt_PDFNew : IHttpHandler, IRequiresSessionState
    {
        ...
        string mailid = string.Empty; int patientid = 0; string PrintRemark = string.Empty;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            mailid = evnt.MailID;
            int.TryParse(evnt.PatientID.ToString(), out patientid);
            UniqueID = evnt.UniqueID;
            int.TryParse(evnt.ReceiptType.ToString(), out ReceiptType);
            FiscalDate = evnt.FiscalDate;
            int.TryParse(evnt.ReceiptType.ToString(), out ReceiptType);
            long ReceiptNo = 0; long.TryParse(evnt.ReceiptNo.ToString(), out ReceiptNo);
            if (string.IsNullOrEmpty(mailid) || string.IsNullOrEmpty(FiscalDate) || string.IsNullOrEmpty(UniqueID) || patientid <= 0 && ReceiptType <= 0 || ReceiptNo <= 0)
            {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                PrintRemark = dt.Rows[0]["Remarks"].ToString();
                ...
                using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
                {
                    ...
                    if (mail)
                    {
                        ...
                        try
                        {
                            ...
                            else if (evnt.Reciever == "2")
                            {
                                receivername = "Father";
                            }
                            else if (evnt.Reciever == "3")
                            {
                                receivername = "Mother";
                            }
                            else if (evnt.Reciever == "4")
                            {
                                receivername = "Reference";
                            }
                            DbHelper.SqlDb dbs = new DbHelper.SqlDb();
                            SqlCommand cmd1 = new SqlCommand("Receipt_Printupdate");
                            cmd1.CommandType = CommandType.StoredProcedure;
                            cmd1.Parameters.Add("@TableID", SqlDbType.VarChar, 10).Value = TableID;
                            cmd1.Parameters.Add("@PatientID", SqlDbType.BigInt).Value = evnt.PatientID;
                            cmd1.Parameters.Add("@Recipttype", SqlDbType.Int).Value = evnt.ReceiptType;
                            cmd1.Parameters.Add("@RecieverName", SqlDbType.VarChar, 20).Value = receivername;
                            cmd1.Parameters.Add("@FullName", SqlDbType.VarChar, 100).Value = FullName;
                            cmd1.Parameters.Add("@mailID", SqlDbType.VarChar, 100).Value = evnt.MailID;
                            cmd1.Parameters.Add("@Remark", SqlDbType.VarChar, 50).Value = PrintRemark;
                            dbs.DbUpdate(cmd1);
